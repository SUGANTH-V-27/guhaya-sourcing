...
interface MidlineInspectionReport {
  id: string;
  styleNumber: string;
  poNumber: string;
  orderQty: number;
...
export default function MidlineInspectionPage({
  params,
}: {
  ...
  const [packingList, setPackingList] = useState<PackingCheckRow[]>([
    { id: "p-1", item: "Packing", isCorrect: true, remarks: "" },
    { id: "p-2", item: "Wash Care", isCorrect: true, remarks: "" },
    { id: "p-3", item: "Price Tag", isCorrect: true, remarks: "" },
    { id: "p-4", item: "Barcode", isCorrect: true, remarks: "" },
    { id: "p-5", item: "Ratio Pack", isCorrect: true, remarks: "" },
    { id: "p-6", item: "2 Pack", isCorrect: true, remarks: "" },
    { id: "p-7", item: "E-Com Pack", isCorrect: true, remarks: "" },
    { id: "p-8", item: "Carton Marking", isCorrect: true, remarks: "" },
    { id: "p-9", item: "Carton Weight", isCorrect: true, remarks: "" },
  ]);
  ...
  function handlePackingToggle(id: string, isCorrect: boolean) {
    setPackingList((prev) =>
      prev.map((p) => (p.id === id ? { ...p, isCorrect } : p))
    );
  }

  function handlePackingRemarks(id: string, remarks: string) {
    setPackingList((prev) =>
      prev.map((p) => (p.id === id ? { ...p, remarks } : p))
    );
  }

  function handleAddPackingRow() {
    setPackingList([
      ...packingList,
      { id: `p-${Date.now()}`, item: "Custom Packaging Item", isCorrect: true, remarks: "" },
    ]);
  }
  ...
  async function handleSaveInspection() {
    const newReport: MidlineInspectionReport = {
      ...
      packedQty: parseInt(packedQty.replace(/,/g, "")) || 0,
      ...
      <div className="space-y-6 text-gray-200 pb-20 max-w-5xl mx-auto">
        ...
        {isSaved && (
          <div className="rounded-xl border border-teal-500/40 bg-teal-500/10 px-4 py-3 text-xs font-semibold text-teal-300 flex items-center justify-between">
            <span>✓ Mid-Line Inspection report saved successfully!</span>
        ...
        {viewMode === "list" && (
          <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              >
                <Plus size={15} /> Create New Mid-Line Inspection
              </button>
            </div>
            ...
            {inspections.length === 0 ? (
              <div className="rounded-2xl border border-gray-800 bg-[#0d1414] py-20 px-4 flex flex-col items-center justify-center text-center space-y-4 shadow-xl">
                ...
                <div className="space-y-1">
                  <h3 className="text-base font-bold text-white">No mid-line inspections yet</h3>
                  <p className="text-xs text-gray-400 max-w-sm">
                    Create the first mid-line inspection report for this style.
                  </p>
                </div>
                  ...
                  >
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-base font-bold text-white">Mid-Line Audit &amp; Packing</h3>
                      </div>
                      <p className="text-xs text-gray-400 mt-1">
                        Output: {insp.outputQty.toLocaleString()} pcs • Packed: {insp.packedQty.toLocaleString()} pcs • Checked: {insp.sampleSize} pcs • Major: {insp.totalMajor}, Minor: {insp.totalMinor}
                      </p>
                    </div>
        ...
        {/* ══════════════════════════════════════════════════════════════════════
            VIEW 2: CREATE MID-LINE INSPECTION FORM (Screenshots 2, 3, 4, 5)
           ══════════════════════════════════════════════════════════════════════ */}
        {viewMode === "create" && (
          <div className="space-y-6">
            {/* Top Back Link */}
            <div>
              ...
              >
                <ArrowLeft size={14} /> Back to Mid-Line Inspections
              </button>
            </div>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-xl bg-violet-900/40 border border-violet-500/40 flex items-center justify-center text-violet-400 font-mono font-black text-sm shrink-0">
                  ML
                </div>
                <div>
                  <h1 className="text-xl font-bold text-white tracking-tight font-serif uppercase">
                    NEW MID-LINE INSPECTION
                  </h1>
                  <p className="text-xs text-gray-400 mt-0.5">
            ...
            <div className="rounded-2xl border border-gray-800 bg-[#0d1414] overflow-hidden shadow-xl">
              <div className="bg-[#2a1b38] px-6 py-3 border-b border-gray-700">
                <h2 className="text-xs font-bold text-gray-200 uppercase tracking-wider">
                  ORDER INFORMATION
                </h2>
              </div>

              <div className="p-6 grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-4 gap-4 text-xs">
                <div>
                ...
                <div>
                  ...
                  <input
                    type="date"
                    value={midlineDate}
                    onChange={(e) => setMidlineDate(e.target.value)}
            ...
            <div className="rounded-2xl border border-gray-800 bg-[#0d1414] p-6 shadow-xl space-y-4">
              <h2 className="text-xs font-bold text-white uppercase tracking-wider">
                AQL STANDARD — PIECES TO CHECK (BASED ON OUTPUT QUANTITY)
              </h2>
            ...
            {/* ── Section 4: PACKING CHECKLIST (Exclusive to Mid-Line!) ───────── */}
            <div className="rounded-2xl border border-gray-800 bg-[#0d1414] overflow-hidden shadow-xl">
              <div className="bg-[#2a1b38] px-6 py-3 border-b border-gray-700">
                <h2 className="text-xs font-bold text-gray-200 uppercase tracking-wider">
                  PACKING CHECKLIST
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left text-xs text-gray-300 border-collapse">
                  <thead className="border-b border-gray-800 bg-black/80 text-[11px] font-bold text-gray-400">
                    <tr>
                      <th className="py-3 px-5 min-w-[220px]">Item</th>
                      <th className="py-3 px-4 text-center w-24">Correct</th>
                      <th className="py-3 px-4 text-center w-24">Incorrect</th>
                      <th className="py-3 px-5 min-w-[280px]">Remarks</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-800/60 font-sans">
                    {packingList.map((item) => (
                      <tr key={item.id} className="hover:bg-gray-800/20 transition">
                        <td className="py-2.5 px-5 font-semibold text-gray-200">{item.item}</td>

                        {/* Correct Radio */}
                        <td className="py-2.5 px-4 text-center">
                          <button
                            type="button"
                            onClick={() => handlePackingToggle(item.id, true)}
                            className={`w-5 h-5 rounded-full border mx-auto flex items-center justify-center transition ${
                              item.isCorrect === true
                                ? "border-emerald-400 bg-emerald-500/20 text-emerald-400 font-bold"
                                : "border-gray-700 bg-transparent text-gray-600 hover:border-gray-500"
                            }`}
                          >
                            {item.isCorrect === true && <Check size={13} strokeWidth={3} />}
                          </button>
                        </td>
                        ...
                        <td className="py-2.5 px-4 text-center">
                          <button
                            type="button"
                            onClick={() => handlePackingToggle(item.id, false)}
                            className={`w-5 h-5 rounded-full border mx-auto flex items-center justify-center transition ${
                          ...
                          >
                            {item.isCorrect === false && <X size={13} strokeWidth={3} />}
                          </button>
                        </td>
                        ...
                        <td className="py-2 px-5">
                          <input
                            ...
                            onChange={(e) => handlePackingRemarks(item.id, e.target.value)}
              ...
              <div className="p-4 border-t border-gray-800/80 bg-black/30">
                <button
                  type="button"
            ...
            {/* ── Section 5: MEASUREMENT CHECK TOGGLE ────────────────────────── */}
                  ...
                  <div className="overflow-x-auto rounded-xl border border-gray-800 bg-black/40">
                    <table className="w-full text-left text-xs text-gray-300">
                      ...
                      <tbody className="divide-y divide-gray-800/60 font-mono">
                        {[
                          { pom: "Total Width (Flat)", spec: "42.0", actual: "42.1", diff: "+0.1", status: "PASS" },
                          { pom: "Total Height", spec: "46.0", actual: "46.0", diff: "0.0", status: "PASS" },
                          { pom: "Handle Drop Length", spec: "28.0", actual: "28.0", diff: "0.0", status: "PASS" },
                        ].map((m, idx) => (
            ...
            <div className="flex justify-end pt-2">
              ...
              >
                <Save size={15} /> Save Mid-Line Inspection
              </button>
            </div>
          </div>
        )}
